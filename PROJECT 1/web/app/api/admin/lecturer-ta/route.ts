...
// Assign a TA to a lecturer (functionality 5).
export async function POST(request: Request) {
  const t = token();
  if (!t) return NextResponse.json({ message: "Not signed in" }, { status: 401 });
  try {
    const body = await request.json();
    return NextResponse.json(await apiSend("/api/admin/lecturer-ta", "POST", t, body));
  } catch (e) {
...
export async function DELETE(request: Request) {
  ...
  try {
    return NextResponse.json(await apiSend(`/api/admin/lecturer-ta/${id}`, "DELETE", t));
  } catch (e) {
    if (e instanceof ApiError) return NextResponse.json({ message: e.message }, { status: e.status });
    return NextResponse.json({ message: "Unable to reach the API" }, { status: 502 });
  }
